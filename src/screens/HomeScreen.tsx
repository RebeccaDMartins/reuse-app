...
export default function HomeScreen({ navigation }: any) {
  ...
  const categories = ["Eletrônicos", "Casa", "Livros", "Moda", "Esportes", "Outros"];
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 48 }}>
        ...
        <Text style={styles.h1} accessibilityRole="header">Olá 👋</Text>
        ...
        <View style={styles.hero} accessible accessibilityLabel="Destaque">
          ...
          <Text style={{ fontSize: 16, fontWeight: "600" }}>Troque, doe ou venda – tudo em um só lugar.</Text>
        </View>
        ...
        <View style={styles.grid}>
          {categories.map((t) => (
            <View key={t} style={styles.gridItem} accessibilityRole="button" accessibilityLabel={`Abrir categoria ${t}`}>
        ...
        {featured.map((t) => (
          <View key={t} style={{ marginTop: 16 }}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {[1,2,3,4].map((i) => (
                <View key={i} style={styles.card} accessibilityRole="summary" accessibilityLabel={`Item ${i}`}>
                  <View style={styles.cardImg} />
                  <Text style={{ fontWeight: "700", marginTop: 6 }}>Item {i}</Text>
                  <Text style={{ color: "#4B5563" }}>R$ 0,00</Text>
                </View>
              ))}
            </ScrollView>
          </View>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", marginTop: 16 }}>
          <OutlineButton label="Explorar" onPress={() => {}} />
          <View style={{ width: 12 }} />
          <PrimaryButton label="Publicar item" onPress={() => navigation.navigate("Publicar")} />
        </View>
      </ScrollView>
    </SafeAreaView>
  );
